...
# Configure cartopy cache BEFORE importing cartopy
CARTOPY_CACHE_DIR = '/var/www/cap_winds_app/.cache/cartopy'
os.makedirs(CARTOPY_CACHE_DIR, exist_ok=True)
os.environ['CARTOPY_DATA_DIR'] = CARTOPY_CACHE_DIR

# Check cache age and refresh if > 28 days old
def check_and_refresh_cartopy_cache():
    """
    Check if cartopy cache is older than 28 days
    If so, clear it to trigger fresh downloads
    """
    cache_path = Path(CARTOPY_CACHE_DIR)
    
    # If cache doesn't exist, nothing to check
    if not cache_path.exists():
        return
    
    # Find oldest file in cache
    try:
        ...
        if age_days > 28:
            print(f"[INFO] Cartopy cache is {age_days} days old (>28 days)")
            print(f"[INFO] Clearing cache to refresh Natural Earth data...")
            
            # Clear cache
            import shutil
            shutil.rmtree(CARTOPY_CACHE_DIR)
            os.makedirs(CARTOPY_CACHE_DIR, exist_ok=True)
            
            print(f"[INFO] Cache cleared - will re-download on first map generation")
        else:
            print(f"[INFO] Cartopy cache is {age_days} days old - using existing data")
    except Exception as e:
        print(f"[WARN] Could not check cache age: {e}")

# Check cache age before importing cartopy
check_and_refresh_cartopy_cache()
...
class MapConfig:
    ...
    # Output directories
    WEB_OUTPUT_DIR = '/var/www/cap_winds_app/static/batch_maps'
    SHAPE_OUTPUT_DIR = '/var/www/cap_winds_app/static/batch_maps'
    ...
    # CAP Region boundaries
    REGION_BOUNDARIES = {
        'NER': {'name': 'Northeast Region', 'bounds': [-73.8, -66.9, 40.5, 47.5]},
        'MAR': {'name': 'Middle Atlantic Region', 'bounds': [-83.7, -73.9, 36.5, 42.3]},
        'GLR': {'name': 'Great Lakes Region', 'bounds': [-91.5, -80.5, 36.9, 49.4]},
        'SER': {'name': 'Southeast Region', 'bounds': [-91.7, -75.4, 24.4, 36.7]},
        'NCR': {'name': 'North Central Region', 'bounds': [-104.1, -89.1, 35.9, 49.4]},
        'RMR': {'name': 'Rocky Mountain Region', 'bounds': [-117.3, -102.0, 36.9, 49.1]},
        'SWR': {'name': 'Southwest Region', 'bounds': [-114.8, -93.5, 25.8, 37.1]},
        'PCR': {'name': 'Pacific Region', 'bounds': [-124.9, -114.0, 32.5, 49.1]},
    }
...
def create_wind_map(wing_code, data, model_run, output_path):
    ...
    elif wing_code in MapConfig.REGION_BOUNDARIES:
        region_info = MapConfig.REGION_BOUNDARIES[wing_code]
        bounds = region_info['bounds']
        title = f"CAP Wind Constraints - {region_info['name']}"
    else:
...
def create_comprehensive_shapefile(model_run, output_dir):
    ...
    """
    log("Creating comprehensive shapefile for all available airports...")
    
    # Query ALL airports with forecast data (no bounds filtering)
    # This works whether we have CONUS-only or full CONUS+OCONUS data
    try:
        conn = get_connection()
        cur = conn.cursor()
        ...
        log(f"Error querying data for shapefile: {e}")
        return False
...
def generate_all_maps():
    ...
    for region_code, region_info in MapConfig.REGION_BOUNDARIES.items():
        log(f"Generating {region_info['name']}...")
        bounds = region_info['bounds']
        ...
        if data:
            output_path = os.path.join(
                MapConfig.WEB_OUTPUT_DIR,
                f"{region_code.lower()}_wind_constraints.png"
            )
            create_wind_map(region_code, data, model_run, output_path)
        else:
            log(f"  No data for {region_code}")
