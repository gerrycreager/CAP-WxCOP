...
Usage:
    ingest_nam_oconus.py --domain ak --cycle 12 --fhour 0 --date 20260611
    ingest_nam_oconus.py --domain hi --cycle 12 --fhour 3 --date 20260611
    ingest_nam_oconus.py --domain carib --cycle 00 --all-fhours --date 20260611
...
# Max forecast hours per domain
MAX_FH = {'ak': 42, 'hi': 27, 'carib': 27}
...
def dpt_from_rh_tmp(rh, tmp_c):
    """Magnus formula dewpoint from RH% and T degC"""
...
def build_impact_row(airport_id, station_id, runway_hdg,
    ...
    return (
        ...
        None,      # tstm_prob -- NULL for NAM OCONUS
        priority,
    )
...
def ingest_file(domain: str, cycle: int, fhour: int, date_str: str = None) -> bool:
    """Ingest one NAM OCONUS grib2 file for given domain/cycle/fhour/date."""

    filename  = FILE_TEMPLATE[domain].format(cycle=cycle, fhour=fhour)

    # Resolve date: explicit --date arg, or today/yesterday based on cycle
    if date_str:
        try:
            run_dt = datetime.strptime(date_str, '%Y%m%d').replace(
                hour=cycle, tzinfo=timezone.utc)
        except ValueError:
            log.error(f'Invalid date: {date_str}')
            return False
    else:
    ...
    # Date-based subdirectory: /LDM/models/nam/ak/20260611/filename
    grib_path = NAM_BASE / domain / date_str / filename

    if not grib_path.exists():
        log.error(f'File not found: {grib_path}')
        return False

    log.info(f'Ingesting {filename} from {date_str} ({grib_path.stat().st_size/1e6:.1f} MB)')
...
def main():
    ...
    ap.add_argument('--date', default=None,
                    help='Date YYYYMMDD of model run (default: most recent)')
    args = ap.parse_args()

    if args.all_fhours:
        max_fh = MAX_FH[args.domain]
        ok = fail = 0
        for fh in range(0, max_fh + 1, 3):
            if ingest_file(args.domain, args.cycle, fh, args.date):
                ok += 1
            else:
                fail += 1
        ...
        success = ingest_file(args.domain, args.cycle, args.fhour, args.date)
