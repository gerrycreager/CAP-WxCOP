...
class Config:
    ...
    def __init__(self):
        ...
        self.MODEL_ROOT = os.getenv('MODEL_ROOT', '/LDM/models/hrrr')
    ...
    def _init_state_boundaries() -> Dict:
        ...
        return {
            ...
            'AK': {'name': 'Alaska', 'bounds': [172, -129, 51, 72], 'region': 'PCR', 'dateline_crossing': True},
...
@dataclass
class WindData:
    """Container for wind analysis results"""
    ...
    init_time: datetime
    data_source: str = "HRRR"  # Default to HRRR, can be "GFS" for OCONUS
...
class HRRRDataManager:
    ...
    def get_current_cycle(self) -> Tuple[datetime, datetime, str, str]:
        ...
        cycle_hour = int(cycle_time.strftime("%H"))
    ...
    def download_grib_file(self, hour: int, cycle_date: str, cycle_hour: str, 
        ...
        fhour = hour
        filename = f"hrrr.t{cycle_hour:02d}z.wrfsfcf{fhour:03d}.grib2"
...
class AirportManager:
    ...
    def _load_airport_data(self) -> pd.DataFrame:
        """Load airport data from PostgreSQL database with caching"""
        if self._airport_cache is None:
            import sys
            sys.path.insert(0, '/var/www/cap_winds_app')
            from db_config import get_connection
            
            _log("Loading airport data from database...")
            
            conn = None
            try:
                conn = get_connection()
                
                # Query airports with all needed fields including is_military
                query = """
                SELECT 
                    station_id as gps_code,
                    station_id as ident,
                    '' as iata_code,
                    name,
                    ST_Y(location) as latitude_deg,
                    ST_X(location) as longitude_deg,
                    elevation_ft,
                    longest_runway_ft,
                    COALESCE(is_military, false) as is_military,
                    CASE 
                        WHEN longest_runway_ft >= 8000 THEN 'large_airport'
                        WHEN longest_runway_ft >= 5000 THEN 'medium_airport'
                        ELSE 'small_airport'
                    END as type,
                    CASE 
                        WHEN iso_region IN ('PR', 'VI', 'GU') THEN iso_region
                        ELSE 'US'
                    END as iso_country,
                    COALESCE(iso_region, '') as iso_region,
                    '' as keywords
                ...
                self._airport_cache = pd.read_sql(query, conn)
                _log(f"Loaded {len(self._airport_cache)} airports from database")
                military_count = self._airport_cache['is_military'].sum()
                if military_count > 0:
                    _log(f"  Including {military_count} military airfields")
                
            except Exception as e:
                _log(f"ERROR loading from database: {e}")
                _log("Falling back to CSV download...")
                # Fallback to original CSV method
                self._airport_cache = pd.read_csv(self.config.AIRPORT_URL)
                self._airport_cache['is_military'] = False
            finally:
                if conn:
                    conn.close()
...
class AirportLabeler:
    ...
    def calculate_priorities(cls, airports_df: pd.DataFrame,
        ...
        # Military installations - use is_military column from database
        if "is_military" in airports_df.columns:
            airports_df.loc[airports_df["is_military"] == True, "priority"] = 1
...
class WindMapBuilder:
    ...
    def create_map(self, map_spec: MapSpec, wind_data: WindData,
        ...
        # For Alaska (crosses dateline), use centered projection
        if map_spec.location_code == 'AK' or map_spec.location_code == 'PCR-AK':
            # Alaska-specific projection centered at 180°W to avoid dateline split
            projection = ccrs.PlateCarree(central_longitude=180)
            _log("Using Alaska-specific projection (centered at 180°W)")
        else:
            projection = ccrs.PlateCarree()
        
        fig = plt.figure(figsize=(20, 12))
        ax = plt.axes([0.1, 0.1, 0.7, 0.8], projection=projection)
    ...
    def _add_airport_labels(self, ax, airports_df: pd.DataFrame, is_conus: bool) -> None:
        """Add labels to selected airports with military priority"""
        airports_sorted = airports_df.sort_values("priority")
        total_airports = len(airports_df)
        
        # Separate by priority
        primary = airports_sorted[airports_sorted["priority"] == 0]
        military = airports_sorted[airports_sorted["priority"] == 1]
        civilian_high = airports_sorted[airports_sorted["priority"] == 2]
        civilian_other = airports_sorted[airports_sorted["priority"] >= 3]
        
        labeled_count = 0
        
        # 1. ALWAYS label primary airport (priority 0) if present
        for _, row in primary.iterrows():
            label = row["gps_code"] if pd.notna(row["gps_code"]) else row["ident"]
            ax.text(
                row["longitude_deg"] + 0.05, row["latitude_deg"] + 0.05, label,
                fontsize=9, fontweight="bold", color="darkred",
                transform=ccrs.PlateCarree(), ha="left", va="bottom", zorder=7,
                path_effects=[path_effects.withStroke(linewidth=3, foreground="white")],
            )
            labeled_count += 1
        
        # 2. ALWAYS label ALL military airfields (priority 1)
        for _, row in military.iterrows():
            label = row["gps_code"] if pd.notna(row["gps_code"]) else row["ident"]
            ax.text(
                row["longitude_deg"] + 0.05, row["latitude_deg"] + 0.05, label,
                fontsize=8, fontweight="bold", color="darkblue",
                transform=ccrs.PlateCarree(), ha="left", va="bottom", zorder=6,
                path_effects=[path_effects.withStroke(linewidth=2, foreground="white")],
            )
            labeled_count += 1
        
        # 3. Label civilian airports based on density limits
        high_priority_civilian = len(civilian_high)
        max_civilian_labels = AirportLabeler.determine_max_labels(
            len(civilian_high) + len(civilian_other), is_conus, high_priority_civilian
        )
        
        civilian_labeled = 0
        
        # Label large civilian airports
        for _, row in civilian_high.iterrows():
            if is_conus and civilian_labeled >= max_civilian_labels:
                break
            
            label = row["gps_code"] if pd.notna(row["gps_code"]) else row["ident"]
            style = AirportLabeler.get_label_style(2)
            ax.text(
                row["longitude_deg"] + 0.05, row["latitude_deg"] + 0.05, label,
                fontsize=style["fontsize"], fontweight=style["fontweight"], 
                color=style["color"], transform=ccrs.PlateCarree(),
        ...
        # Label medium/small civilian if space available (non-CONUS only)
        if not is_conus:
            for _, row in civilian_other.iterrows():
                if civilian_labeled >= max_civilian_labels:
                    break
    ...
    def _add_annotations(self, fig, ax, map_spec: MapSpec,
        ...
        if map_spec.location_code == 'AK' or map_spec.location_code == 'PCR-AK':
            # Alaska crosses dateline: 172°E to 129°W
            # With central_longitude=180, shift coordinates
            # 172°E → 172-180 = -8°
            # -129°W → -129-(-180) = 51° (or -129+180 = 51°)
            padding = 0.5
            west_bound = map_spec.bounds[0] - 180  # 172 - 180 = -8
            east_bound = map_spec.bounds[1] + 180  # -129 + 180 = 51
            ax.set_extent(
                [west_bound - padding, east_bound + padding,
                 map_spec.bounds[2] - padding, map_spec.bounds[3] + padding],
                crs=ccrs.PlateCarree(central_longitude=180),
            )
        else:
            # Normal bounds handling
        ...
        # Add title with dynamic data source
        data_source_text = "NOAA GFS (US Gov't)" if wind_data.data_source == "GFS" else "NOAA HRRR (US Gov't)"
        plt.suptitle(
...
class WindAnalysisService:
    ...
    def _create_map_spec(self, location_type: str, location_code: str,
        ...
        if location_type == "radius":
            ...
            if location_code not in self.config.STATE_BOUNDARIES:
                raise ValueError(f"Unknown wing code: {location_code}")
            wing_info = self.config.STATE_BOUNDARIES[location_code]
            location_name = wing_info["name"]
    ...
    def generate_analysis(self, location_type: str, location_code: str = "",
        ...
        Args:
            location_type == 'wing', 'region', 'conus', or 'radius'
