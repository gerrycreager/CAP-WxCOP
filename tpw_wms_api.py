"""
tpw_wms_api.py — Flask blueprint for GOES ABI L2 Total Precipitable Water
MapServer WMS proxy + frame index.

Endpoints:
  GET /CAP_WxCOP/api/tpw/frames?product=tpw_conus_west&hours=3
      Returns JSON list of available cached timestamps for the product.

  GET /CAP_WxCOP/api/tpw/wms?product=tpw_conus_west&ts=20260712-130118
                            &SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap
                            &LAYERS=tpw_conus_west&SRS=EPSG:4326
...
See tpw_cache_updater.py for how these six products are produced -- routing
is by each source file's own scene_id/orbital_slot, not by which WMO header
it arrived under (that header is not a reliable indicator).
...
tpw_wms_bp = Blueprint('tpw_wms', __name__)
...
PRODUCT_CONFIG = {
    'tpw_conus_west': {'mapfile': 'tpw_conus_west.map', 'layer': 'tpw_conus_west'},
    'tpw_conus_east': {'mapfile': 'tpw_conus_east.map', 'layer': 'tpw_conus_east'},
    'tpw_full_west':  {'mapfile': 'tpw_full_west.map',  'layer': 'tpw_full_west'},
    'tpw_full_east':  {'mapfile': 'tpw_full_east.map',  'layer': 'tpw_full_east'},
    'tpw_meso_west':  {'mapfile': 'tpw_meso_west.map',  'layer': 'tpw_meso_west'},
    'tpw_meso_east':  {'mapfile': 'tpw_meso_east.map',  'layer': 'tpw_meso_east'},
}
...
@tpw_wms_bp.route('/api/tpw/frames')
def tpw_frames():
    """
    Return sorted list of available frame timestamps for a product.

    Query params:
      product : tpw_conus_west | tpw_conus_east | tpw_full_west | tpw_full_east |
                tpw_meso_west | tpw_meso_east (required)
      hours   : 1-36 (default 3)

    Response:
      {
        "product": "tpw_conus_west",
        "hours": 3,
        "frames": [
          {"ts": "20260712-125618", "label": "-30 min"},
          ...
          {"ts": "20260712-130118", "label": "LIVE"}
        ]
      }
    """
...
@tpw_wms_bp.route('/api/tpw/wms')
def tpw_wms():
    """
    MapServer WMS proxy for a specific cached TPW frame.
