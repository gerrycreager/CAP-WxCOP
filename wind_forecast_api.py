...
REGION_STATES = {
    ...
    # Northeast Region (NER covers New England only)
    'NER':       ['US-CT','US-MA','US-ME','US-NH','US-RI','US-VT'],
    # Southeast Region — CONUS states
    'SER':       ['US-AL','US-FL','US-GA','US-MS','US-NC','US-SC','US-TN'],
    'SER-CONUS': ['US-AL','US-FL','US-GA','US-MS','US-NC','US-SC','US-TN'],
    # Southwest Region
    'SWR':       ['US-AR','US-AZ','US-LA','US-NM','US-OK','US-TX'],
    # Rocky Mountain Region
    'RMR':       ['US-CO','US-ID','US-MT','US-UT','US-WY'],
    # Pacific Region CONUS
    'PCR':       ['US-CA','US-NV','US-OR','US-WA'],
    'PCR-CONUS': ['US-CA','US-NV','US-OR','US-WA'],
}

# Bounding boxes for CONUS overview and OCONUS territories
REGION_BOUNDS = {
    'CONUS':    {'west': -125,  'south': 24,   'east': -66,   'north': 50  },
    'SER-PR':   {'west': -67.3, 'south': 17.6, 'east': -64.6, 'north': 18.6},
    'PCR-AK':   {'west': -180,  'south': 51,   'east': -129,  'north': 72  },
    'PCR-HI':   {'west': -160.3,'south': 18.9, 'east': -154.7,'north': 22.3},
    'PCR-GU':   {'west': 144.5, 'south': 13.1, 'east': 145.1, 'north': 13.8},
    # Legacy alias kept for any existing routes
    'PCR-GUAM': {'west': 144.5, 'south': 13.1, 'east': 145.1, 'north': 13.8},
}
...
def get_wind_forecasts_by_states(state_list, limit=5000):
    """
    Query wind forecasts for a list of iso_region codes.
    Returns (airports_list, bounds_dict).
    """
    try:
        conn = get_connection()
        cur  = conn.cursor()
...
def generate_wind_grid(airports, bounds, grid_resolution=50):
    ...
    try:
        grid_wind = griddata((lons, lats), winds,
                             (grid_lon, grid_lat), method='cubic', fill_value=np.nan)
    except:
        grid_wind = griddata((lons, lats), winds,
    ...
    if mask.any():
        grid_wind[mask] = griddata((lons, lats), winds,
                                   (grid_lon, grid_lat), method='nearest')[mask]
...
def get_region_forecasts(region_code):
    ...
    if region_code in REGION_STATES:
        ...
        if not airports:
            log.warning(f"State query empty for {region_code}, falling back to bbox")
            bounds   = REGION_BOUNDS.get(region_code,
                                         {'west': -125, 'south': 24, 'east': -66, 'north': 50})
...
def get_state_forecasts(state_code):
    ...
    state_bounds = {
        ...
        'GU': {'west': 144.5,  'south': 13.1, 'east': 145.1,  'north': 13.8},
    }
